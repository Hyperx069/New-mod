...
//				});
		...
		bloodStar = new ItemTurret("blood-star"){{
			size = 5;
			coolant = consumeCoolant(0.2F);
			requirements(Category.turret, BuildVisibility.shown, with(NHItems.zeta, 300, NHItems.seniorProcessor, 20, NHItems.presstanium, 30, Items.thorium, 300));
			recoil = 5f;
			reload = 150f;
			range = 720f;
...
//			heatColor = NHItems.irayrondPanel.color;
		...
		executor = new ItemTurret("executor"){{
			...
			health = 15200;
			armor = 10;
			requirements(Category.turret, BuildVisibility.shown, with(NHItems.irayrondPanel, 650, NHItems.seniorProcessor, 150, NHItems.multipleSteel, 40));
			ammo(
...
//					alpha = 0.76f;
		...
		ancimembraneConcentrator = new AttributeCrafter("ancimembrane-concentrator"){{
			...
			consumePower(12);
			consumeItems(with(NHItems.setonAlloy, 2, Items.titanium, 150));
			consumeLiquid(NHLiquids.quantumEntity, 0.2f);
			outputItems = with(NHItems.ancimembrane, 20);
//			outputLiquid = new LiquidStack(Liquids.water, 0.5f);
		...
		multiplePresstaniumFactory = new GenericCrafter("multiple-presstanium-factory"){{
			...
			outputItems = with(NHItems.presstanium, 15);
		...
		fusionEnergyFactory = new GenericCrafter("fusion-core-energy-factory") {
			{
				...
				craftEffect = Fx.smeltsmoke;
				outputItem = new ItemStack(NHItems.fusionEnergy, 12);
				craftTime = 90f;
				size = 3;
				itemCapacity = 20;
				liquidCapacity = 60f;
		...
		seniorProcessorFactory = new GenericCrafter("processor-senior-factory") {
			{
				...
				hasPower = hasItems = true;
				drawer = new DrawMulti(new DrawDefault(), new DrawFlame(NHItems.fusionEnergy.color));
				consumeItems(new ItemStack(Items.surgeAlloy, 2), new ItemStack(NHItems.juniorProcessor, 2));
				consumePower(6f);
			}
		};
		...
		metalOxhydrigenFactoryLarge = new GenericCrafter("metal-oxhydrigen-factory-large"){{
			...
			outputItem = new ItemStack(NHItems.metalOxhydrigen, 12);
			craftTime = 60f;
			hasPower = hasLiquids = hasItems = true;
			drawer = new DrawMulti(new DrawRegion("-bottom"), new DrawArcSmelt(){{
				flameRad = 0.45f;
		...
		zetaFactoryLarge = new GenericCrafter("large-zeta-factory") {{
			...
			outputItem = new ItemStack(NHItems.zeta, 12);
			craftTime = 30f;
			size = 2;
			craftEffect = Fx.formsmoke;
		...
		multipleSteelFactory = new GenericCrafter("multiple-steel-factory") {
			{
				...
				outputItem = new ItemStack(NHItems.multipleSteel, 20);
				craftTime = 60f;
				itemCapacity = 20;
				health = 600;
				size = 3;
				...
				consumeItems(new ItemStack(Items.thorium, 8), new ItemStack(Items.titanium, 2));
				consumePower(3f);
			}
		};
		...
		reconstructPlastaniumFactory = new GenericCrafter("reconstruct-plastanium-factory"){{
			...
			outputItem = new ItemStack(Items.plastanium, 20);
			craftTime = 120f;
			itemCapacity = 24;
			liquidCapacity = 20f;
			health = 1200;
		...
		setonAlloyFactory = new GenericCrafter("seton-alloy-factory"){{
			...
			outputItem = new ItemStack(NHItems.setonAlloy, 9);
			craftTime = 90f;
			itemCapacity = 24;
			liquidCapacity = 20f;
			health = 500;
			...
			consumeItems(new ItemStack(Items.plastanium, 3), new ItemStack(Items.graphite, 6));
			consumePower(12f);
		}};
	...
	private static void loadPowers(){
		...
		armorBatteryLarge = new Battery("large-armor-battery"){{
			...
			health = 3000;
			armor = 15;
			consumePowerBuffered(800000.0F);
		}};
